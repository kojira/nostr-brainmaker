...
4. **verify** the browser asset set actually landed (required:
   `manifest.json`, the production `onnx/model_quantized.onnx`, `tokenizer.json`,
   `config.json`; recommended: `tokenizer_config.json`,
   `special_tokens_map.json`, `label_map.json`).

The production path is **q8-only**: the deployed manifest targets
`onnx/model_quantized.onnx` with `dtype q8`, and the deploy hard-errors if that
int8 artifact is missing — there is **no fp32/q4 production fallback**. The fp32
(`onnx/model.onnx`) and q4 (`onnx/model_q4.onnx`, dtype q4) artifacts are
...
npm run model:deploy -- <train-output-run-dir>              # q8 (production default; int8)
npm run model:deploy -- <train-output-run-dir> --dev-fp32   # DEV ONLY: unquantized fp32
npm run model:deploy -- <train-output-run-dir> --dev-q4     # DEV ONLY: 4-bit weight-only model, manifest dtype q4
...
Flags: q8 (`onnx/model_quantized.onnx` with `dtype q8`) is the production default
and requires the int8 artifact; `--dev-q4` (`onnx/model_q4.onnx` with `dtype q4`)
and `--dev-fp32` (`onnx/model.onnx`) are dev-only alternatives;
`--skip-export`, `--opset <n>` (default 18), `--python <bin>`
...
The pure decision logic (arg parsing, q8-vs-fp32-vs-q4 artifact selection, the
