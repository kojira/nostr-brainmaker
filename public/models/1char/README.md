...
- `onnx/model_quantized.onnx` — 本番用 ONNX モデル本体（int8 量子化）。`manifest.json` の `model.files.model` で参照（本番既定 `onnx/model_quantized.onnx`、`model.dtype` は `q8`）。
- 任意（非本番の代替）で `onnx/model.onnx`（fp32）や `onnx/model_q4.onnx`（4bit weight-only）。fp32 を使う場合は `model.dtype` を `null`、4bit を使う場合は `model.dtype` を `q4`、`model.files.model` をそれぞれのファイル名にする。
...
#    本番は int8 量子化版（onnx/model_quantized.onnx / dtype q8）を成果物とする。
pip install -r finetune_smoke/requirements-export.txt
npm run model:deploy -- finetune_smoke/train-output/run-<ts>
#   非本番の代替（開発用のみ）: 4bit weight-only 版（dtype q4）を出す場合:
npm run model:deploy -- finetune_smoke/train-output/run-<ts> --dev-q4
#   何を実行するか確認だけする場合:
...
#   本番（q8 / int8）: node scripts/build-model-manifest.js <run-dir> --dtype q8 --model-file onnx/model_quantized.onnx
node scripts/build-model-manifest.js finetune_smoke/train-output/run-<ts>
#   非本番の代替（4bit）: python3 finetune_smoke/export_onnx.py --run-dir <run-dir> --q4 ; node scripts/build-model-manifest.js <run-dir> --dtype q4 --model-file onnx/model_q4.onnx
```
